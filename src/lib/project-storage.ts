...
const appearanceValueValidators: {
  ...
  dotColor: (raw) => strictHexColor(raw),
  lineColor: (raw) => strictHexColor(raw),
  graphMajorColor: (raw) => strictHexColor(raw),
  borderColor: (raw) => strictHexColor(raw),
  numberColor: (raw) => strictHexColor(raw),
  graphCompleteBlocks: (raw) => strictBoolean(raw),
  numberBold: (raw) => strictBoolean(raw),
  numberItalic: (raw) => strictBoolean(raw),
  numberFont: (raw) => (typeof raw === "string" && raw.trim() !== "" ? raw : undefined),
  dotSize: (raw) => strictNumber(raw),
  dotSpacing: (raw) => strictNumber(raw),
  dotMajorEvery: (raw) => strictNumber(raw),
  dotMajorSize: (raw) => strictNumber(raw),
  lineWidth: (raw) => strictNumber(raw),
  lineSpacing: (raw) => strictNumber(raw),
  fourLineGap: (raw) => strictNumber(raw),
  slantAngle: (raw) => strictNumber(raw),
  graphMajorEvery: (raw) => strictNumber(raw),
  graphMajorLineWidth: (raw) => strictNumber(raw),
  margin: (raw) => strictNumber(raw),
  gutterMargin: (raw) => strictNumber(raw),
  borderWidth: (raw) => strictNumber(raw),
  numberFontSize: (raw) => strictNumber(raw),
}
